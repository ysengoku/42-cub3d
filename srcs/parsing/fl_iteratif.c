...
/*   Updated: 2024/06/21 17:44:26 by jmougel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	apply_cross_player(t_cub3d *data, char **dup_map, int x, int y)
{
	...
	{
		if (apply_on_pos(dup_map, x, y - 1) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	}
	else
		return (EXIT_FAILURE);
	...
	{
		if (apply_on_pos(dup_map, x - 1, y) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	}
	else
		return (EXIT_FAILURE);
...
void	flood_fill_iter(t_cub3d *data, char **dup_map)
{
	...
	{
		if (BONUS && find_treasure(data, dup_map, data->map.check.x, data->map.check.y) == EXIT_SUCCESS)
			data->map.check.catch_treasure = true;
		if (apply_cross_player(data, dup_map, data->map.check.x, data->map.check.y) == EXIT_FAILURE)
		{
